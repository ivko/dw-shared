(function ($) {
  ...
  $.widget("DocuWare.permissionPlugin", {
    options: {
      captions: {
        addAll: " ++ ",
        removeAll: " -- ",
    ...
    _create: function () {
      ...
      this.tabsContainerSource = $('<div id="tabsSource"></div>').appendTo(this.availableContainer);
      this.tabsContainerSourceButtons = $('<ul></ul>').appendTo(this.tabsContainerSource);

      this.tabsContainerTarget = $('<div id="tabsTarget"></div>').appendTo(this.selectedContainer);
      ...
      this.optGroups.each(function (i, el) {
        //add for every optgroup header, an equivalent button in Tabs container
        $("<li><a href='#sourceTab-" + i + "'>" + el.label + "</a></li>").appendTo(self.tabsContainerSourceButtons);

        //now we've got to put all elments of an optgroup into the tabs
        oneTab = $("<div id='sourceTab-" + i + "'></div>").appendTo(self.tabsContainerSource);
        console.log(self.options.highCountTrigger);
        var placeHolder = (self.options.highCountTrigger) ? 'placeholder="search for entity"' : '';

        $('<div class="actions ui-widget-header ui-helper-clearfix"><input type="text" '+ placeHolder + ' class="search empty ui-widget-content ui-corner-all"/><a href="#" class="add-all">' + self.options.captions.addAll + '</a></div>').appendTo(oneTab);
        availableList = $('<ul class="available connected-list"><li class="ui-helper-hidden-accessible"></li></ul>').bind('selectstart', function () { return false; }).appendTo(oneTab);
        availableList.height(self.options.height);

        //bind to every option the parent <ul>
        $("option", el).each(function (index, opt) {
          $(opt).data("parentTab", availableList)
        });
      });

      var selectedList;
      this.targetEntries.each(function (i, el) {
        $("<li><a href='#targetTab-" + i + "'>" + $(el).text() + "</a></li>").appendTo(self.tabsContainerTargetButtons);

        oneTab = $("<div id='targetTab-" + i + "'></div>").appendTo(self.tabsContainerTarget);
        ...
        selectedList = $('<ul class="selected connected-list"><li class="ui-helper-hidden-accessible"></li></ul>').bind('selectstart', function () { return false; }).appendTo(oneTab);
        selectedList.height(self.options.height);

        $(el).data("linkedTargetTab", oneTab.attr("id"))
      });
      ...
      // 0 pad version number for string compare
      var version = $.ui.version.split('.').map(function (val) {
          return ('0' + val).slice(-2);
      }).join('.');

      var sourceTabOptions = version >= "01.10" ? {
        beforeActivate: function (event, ui) {
          ui.panel = ui.newPanel[0];
      ...
      var targetTabOptions = version >= "01.10" ? {
        beforeActivate: function (event, ui) {
          ui.panel = ui.newPanel[0];
      ...
      //activate Tabs
      this.tabsContainerSource.tabs(sourceTabOptions);
      this.tabsContainerTarget.tabs(targetTabOptions);
      ...
      this.container.find(".remove-all").click(function () {
        ...
        $(".selected #" + activeTargetTabId + " ul .ui-element").each(function (i, el) {
      ...
      this.container.find(".add-all").click(function () {
        ...
        $(".available #" + activeSourceTabId + " ul li.ui-element:not([style*='display: none'])").each(function (i, el) {
    ...
    _refreshAvailableList: function () {
        var self = this;
        //remove available
        $(".available ul").each(function (i, el) { $(el).children('.ui-element').remove(); })
        $('<div class="available"></div>').children().each($(this).remove());
        ...
        var items = $(options.map(function (i) {
            ...
            try {
                ...
                linkedTargetTab = $("#" + $("#" + selectedIn).data("linkedTargetTab") + " ul");
                //and add entry to linked or active tab
                var item = that._getOptionNode(this, targetList).appendTo(this.isSelected ? linkedTargetTab : $(this).data("parentTab")).show()
            } catch (e) {
                this.isSelected = false; //revert back to non-selected if targetTabId couldn't be found
                var item = that._getOptionNode(this, targetList).appendTo($(this).data("parentTab")).show()
            }
    ...
    _switchTab: function () {
      ...
      $(".available ul .ui-element").each(function (i, el) {
    ...
    _populateLists: function (options, isRemoteSearch, serverResults) {
        if (isRemoteSearch == undefined)
            isRemoteSearch = false;
      //first of all remove all items in any tab, they will be added through current selection
      //or init mapping
      $(".available ul").each(function (i, el) { $(el).children('.ui-element').remove(); })
      if (!isRemoteSearch) {
          $(".selected ul").each(function (i, el) { $(el).children('.ui-element').remove(); })
      }
      ...
      targetLists.each(function (index, el) {
        ...
        var items = $(options.map(function (i) {
          ...
          try {
              var relatedUL = (isRemoteSearch) ? $("#sourceTab-" + targetList + " ul") : $(this).data("parentTab")

              //check also, if linkedTargetTab(attribute: "selectedin") does really exist
              linkedTargetTab = $("#" + $("#" + selectedIn).data("linkedTargetTab") + " ul");
              //and add entry to linked or active tab
              var item = that._getOptionNode(this, targetList).appendTo(this.isSelected ? linkedTargetTab : relatedUL).show()
          } catch (e) {
              this.isSelected = false; //revert back to non-selected if targetTabId couldn't be found
              var item = that._getOptionNode(this, targetList).appendTo(relatedUL).show()
          }
    ...
    _getOptionNode: function (option, connectedTo) {
      option = $(option);
      var node = this._createNode(option.text()).hide();
    ...
    _createNode: function (text) {
        //return $('<li class="ui-state-default ui-element" title="' + text + '"><span class="ui-icon"/>' + text + '<a href="#" class="action"><span class="ui-corner-all ui-icon"/></a></li>');
        return $('<li class="ui-state-default ui-element" title="' + text + '"><span class="ui-helper-hidden"></span>' + text + '<a href="#" class="action"><span class="ui-corner-all ui-icon ui-icon-plus"></span></a></li>');
    },
    ...
    _setSelected: function (item, selected) {
      ...
      if (selected) {
        ...
        var items = $("li", $(".available"))

        var availableItem = this._cloneWithData(item);
        var tabOrigin = availableItem.data('optionLink').data("parentTab");
    ...
    _registerSearchEvents: function (input) {
      ...
      input.focus(function () {
        $(this).addClass('ui-state-active');
		    ...
		    .keyup(function (e) {
		        if (e.keyCode == 13 && self.options.highCountTrigger)
		            self._getResultsFromRemote(self.availableList, jQuery.data(self.activeTargetTab.panel, "activeTargetTabId"));
		        else
		      self._filter.apply(this, [self.availableList, jQuery.data(self.activeTargetTab.panel, "activeTargetTabId")]);
		    });
    },
    _getResultsFromRemote: function (availableList, activeTargetTab) {
        var resultsFromServer = this.options.callbackfunction($('.search').val().toLowerCase());
        //var options = $("option", $("optgroup", $(this.element).find("select")));
        var visibleTabId = $("#tabsSource").tabs("option", "selected");
        var selectedSourceTabId = "#sourceTab-" + visibleTabId;
        var selectedListContainer = $("ul", selectedSourceTabId);

        //remove all entries from visible UL < LI list
        var selectedVisible = $("li.ui-element", selectedSourceTabId);
        selectedVisible.remove();

        //add LI for new element into visible UL
        var permissionPlugin = this;
        if (resultsFromServer.length > 0) {
            $(resultsFromServer).each(function (i, el) {
                var itemForResult = permissionPlugin._createNode(el.Name).appendTo(selectedListContainer);
                var plusIcon = $("a.action", itemForResult);

                //create instance of OPTION into corresponding OPTGROUP and set 
                // * GUID as "value", 
                // * "selectedin" attribute
                // * "allowedin" attribute
                var optGroup = $("select").find("optgroup#" + visibleTabId);
                var newOption = $("<option />").val(el.Guid).text(el.Name).attr("selectedin", "").attr("locked", "0").attr("allowedin", "0,1").appendTo(optGroup);
                itemForResult.data("optionLink", newOption);

                //register events for hover, click(+) & double-click
                permissionPlugin._registerHoverEvents(itemForResult);
                permissionPlugin._registerDoubleClickEvents(itemForResult);
                permissionPlugin._registerAddEvents(plusIcon);
            });
        }
