function PermissionControllerUser(customOptions) {
    var self = this;
    self.plugin;
    var options = {
        pluginOptions: {
        ...
        //Real values are passed from callerplugin
        entityTypes: [
            {
                "Id": 0,
                "Name": "User"
    ...
    self.lock = function () {
        if (!self.hasLock())
            self.plugin.permissionPluginUser('option', 'locked', true);
    }

    self.unlock = function () {
        if (self.hasLock())
            self.plugin.permissionPluginUser('option', 'locked', false);
    }

    self.hasLock = function () {
        return self.plugin.permissionPluginUser("option", "locked");
    }

    self.render = function () {
        //reset plugin if already exists
        if (self.plugin != undefined) {
            self.plugin.permissionPluginUser('destroy');
        }

        // enable permissions control
        self.plugin = $(options.selector).permissionPluginUser(options.pluginOptions);
    }
