    ...
    var createDelegatedHandler = function(eventName, root, bubble) {
        return function(event) {
            var data, method, context, action, owner, matchingParent, command, result,
                el = event.target || event.srcElement, //the element that was clicked on
                attr = "data-" + eventName, //data-click
                key = prefix + eventName; //ko_delegated_click
            ...
            if (method) {
                ...
                if (context) {
                    ...
                    if (typeof method === "string") {
                        ...
                        if (!action) {
                            var objectsToSearchIn = context.$parents;
                            context.$parents.forEach(function (parent) {
                                var subParents = Object.keys(parent).map(function (key) {
                                    return parent[key];
                                });
                                objectsToSearchIn = objectsToSearchIn.concat(subParents);
                            });

                            matchingParent = ko.utils.arrayFirst(objectsToSearchIn, function(parent) {
