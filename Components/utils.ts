...
namespace DW.Utils {
  ...
  */
    ...
    * @return {dfd.resolve();}
    */
    export let resolvedDeferred: JQueryDeferred<any>;

    /**
    * Create and return rejected deferred
    * @return {dfd.reject();}
    */
    export let rejectedDeferred: JQueryDeferred<any>;
    ...
    */
    export function resolvedDeferredWithParam<T>(param: T): JQueryDeferred<T> {
        return DW.Deferred(function (dfd) { dfd.resolve(param); });
    };
    ...
    */
    export function rejectedDeferredWithParam(param: any): JQueryDeferred<any> {
        return DW.Deferred(function (dfd) { dfd.reject(param); });
    };
    ...
    */
     ...
     * @param value
     * @return the parsed value, or the value itself if the value meets the criteria in DW.Utils.isRealObject
     */
    export function parseFloat(value: any): any {
        //window is nesessary here to prevent recursion
        return DW.Utils.isRealObject(value) ? window.parseFloat(value) : value;
    }
   ...
   */
    ...
    /**
    * whenAllDone is wrapper around DW.When to wait for all tasks, nevertheless if any of the tasks fails.
    * By default DW.When rejects, when any of the passed tasks fails and it doesn't wait for the rest to finish.
    ...
    */
    export function whenAllDone(...args: any[]): JQueryPromise<any> {
        var result = DW.Deferred(),
            failed = false,
            _arguments = arguments.length && $.isArray(arguments[0]) ? arguments[0] : arguments,
            num = _arguments.length;

        // wrap all passed tasks to always finish with success, so that DW.When waits for all of them
        var deferreds = $.map(_arguments, function (current) {
            var wrapDeferred = DW.Deferred();
            current
                .always(function () {
        ...
        DW.When.apply(null, deferreds).done(function () {
    ...
    */
    export function nextId(postfix?: string): string {
        postfix = postfix || '';
        return 'dwnid_' + (DW.Utils._nextId++) + '_' + postfix;
    };
    ...
    */
    ...
    export function debounceWithResult<T>(func: () => T, wait: number, immediate?: boolean): () => JQueryPromise<T> {
        let executing = { deferred: DW.Deferred<T>() },
            wrapper = (...args) => {
                executing.deferred.resolve(func.apply(this, args));
            },
            debounced = DW.Utils.debounce(wrapper, wait, immediate);
        return () => {
            debounced();

            return executing.deferred.promise()
                .then((result) => {
                    executing.deferred = DW.Deferred();
    ...
    */
    ...
    export function virtualProxy(job, context?): any {
        ...
        return function (force) {
            if (!task || (force && task.state() != DW.Utils.DeferredStates.pending)) {
                //create or renew current initializing task 
                task = DW.Deferred();
                DW.When(job.apply(context, Array.prototype.slice.call(arguments)))
    ...
    */
    export function wrapClassParentMethod(owner: any, args: any): any {
        ...
        //    return DW.Deferred(function (task) {
    ...
    */
    export function copyIterativeName(
        ...
        let result = DW.Utils.format(namePrefix, name);
        ...
        if (elementsWithName && elementsWithName.length > 0) {
            ...
            while (j <= elementsWithName.length) {
                ...
                result = DW.Utils.format(nameSuffix, name, ++j);
            }
        }
        return result;
    };
   ...
   * @return { void }
   */
    export function dispose(obj: any | DWTS.IDisposable | DWTS.IDisposable[]): any {
        if (obj) {
            return $.isFunction((<DWTS.IDisposable>obj).dispose) ?
                (<DWTS.IDisposable>obj).dispose() :
                $.isArray(obj) && (<DWTS.IDisposable[]>obj).forEach((disposable) => $.isFunction(disposable.dispose) && disposable.dispose());
        }
    };
    ...
    */
    export function getScrollbarSize(): { height: number, width: number } {
        return DW.Utils._scrollbarSize;
    };
    ...
    */
    export function execSequentially(first: any, ...args: any[]): JQueryPromise<any> {
        if (arguments.length === 0) {
            ...
            return DW.When(first.call()).then((result) => {
                return (result !== false) ? result : DW.Deferred().reject().promise();
            });
        } else {
            var rest = [].slice.call(arguments, 1);
            return DW.Utils.execSequentially(first).then(() => {
    ...
    */
    export function getNormalizedDate(date: Date): Date {
        return date ? DW.DateTime.normalizeDate(new Date(date.getTime())) : null;
    };
    ...
    */
     ...
     * @param okButtonText
     */
    export function messageBox(title: string, message: string, okButtonText: string = $R('Button_OK_Text')) {
    ...
    export function fixScheduleOnTimeDateTimeForSs(onTime: dev.docuware.com.settings.common.dwschedule.ScheduleOnTime):
        ...
        if (onTime) {
            ...
            if (schedule) {
                schedule.StartAt = <any>DW.DateTime.getDateForSS(new Date(schedule.StartAt));
                if (schedule.EndAt) {
                    schedule.EndAt = <any>DW.DateTime.getDateForSS(new Date(schedule.EndAt));
                }
            }
        }
    ...
    export function fixScheduleOnTimeDateTimeFromSs(onTime: dev.docuware.com.settings.common.dwschedule.ScheduleOnTime):
        ...
        if (onTime) {
            ...
            if (schedule) {
                schedule.StartAt = DW.DateTime.getDateFromSSDate(schedule.StartAt);
                if (schedule.EndAt) {
                    schedule.EndAt = DW.DateTime.getDateFromSSDate(schedule.EndAt);
    ...
    */
    ...
    export function getColors(): Array<{ argb: string, hex: string }> {
        var result = [];
        for (var dwColor in DW.Utils.argbColorsToHex) {
            result.push({
                hex: DW.Utils.argbColorsToHex[dwColor],
                argb: dwColor
            });
        }
        return result;
    ...
    */
    ...
    export function folderDropIsSupported() {
        return DW.Utils.isChrome || DW.Utils.isFF || DW.Utils.isEdge;
    };
    ...
    export function isElementOverflowing(text, el) {
        var $el = $(el),
            $contentAsElement = $('<div>' + DW.Utils.htmlEncode(text) + '</div>'),
            isOverflowing;
    ...
    export function copyToClipboard(text: string, containerSelector = "body"): JQueryPromise<any> {
        ...
        if (!success && (<any>navigator).permissions) { //supported in FF and Chrome
            (<any>navigator).permissions.query({ name: "clipboard-write" }).then(
                ...
                result => {
                    if (DW.Utils.isFF) { //Firefox does not yet support the "clipboard-read" and "clipboard-write" permissions (this else if should be removed in the future)
...
(function (factory) {
    if (typeof define === "function" && define.amd) { // AMD.
        define(['./Deferred'], factory);
    } else { // Global
        factory();
    }
}(function () {
    DW.Utils.resolvedDeferred = DW.Deferred(function (dfd) { dfd.resolve(); });
    DW.Utils.rejectedDeferred = DW.Deferred(function (dfd) { dfd.reject(); });
}));
//Array extensions
