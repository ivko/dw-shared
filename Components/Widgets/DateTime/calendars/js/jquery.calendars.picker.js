...
$.extend(CalendarsPicker.prototype, {
	...
	_generateMonthSelection: function(inst, year, month, minDate, maxDate, monthHeader, calendar) {
		...
		if (yearRange == 'any') {
            ...
            //NN use the correct valid for the calendar year, 
            //otherwise calendar.newDate(start + 1, calendar.firstMonth, calendar.minDay).add(-1, 'd');
            //will throw exception
            start = minDate ? Math.max(start, minDate.year()) : start;
			var end = (yearRange[1].match('c[+-].*') ? year + parseInt(yearRange[1].substring(1), 10) :
                ((yearRange[1].match('[+-].*') ? todayYear : 0) + parseInt(yearRange[1], 10)));
            end = maxDate ? Math.min(end, maxDate.year()) : end;
