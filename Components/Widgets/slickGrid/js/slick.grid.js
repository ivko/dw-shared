...
(function ($) {
    ...
    function SlickGrid(container, data, columns, options) {
        ...
        function createColumnHeaders() {
            ...
            for (var i = 0; i < columns.length; i++) {
                ...
                var header = $("<div class='ui-state-default slick-header-column' />")
                    ...
                    .outerWidth(m.width - headerColumnWidthDiff) //changed from width() to outerWidth() because of bootstrap box-sizing:border-box applied to all divs in the DOM
                    .attr("id", "" + uid + m.id)
                ...
                if (options.enableColumnReorder || m.sortable || m.filterable) {
                    header
                ...
                if (m.filterable) {
                    header.addClass("slick-header-filterable");
                    header.append("<span class='slick-filter-indicator'/>"); // ui-icon button ui-icon-filter
        ...
        function setupColumnClickSubscriptions() {
            $headers.click(function (e) {
                ...
                if (column.filterable) {
                    setupColumnFilter(e, column);
                }

                if (column.sortable) {
                    setupColumnSort(e, column);
                }
            });
        }

        function setupColumnSort(e, column) {
            if (!$(e.target).hasClass("slick-filter-indicator")) {
        ...
        function setupColumnFilter(e, column) {
            setFilterColumns(filterColumns);
            if ($(e.target).hasClass("slick-filter-indicator")) {

                trigger(self.onFilter,
                    {
                        getFilterCols: function () {
                            return filterColumns;
                        },
                        setValue: function (value) {
                            if (!value && filterColumns[column.id])
                                delete filterColumns[column.id];
                            else if (value){
                                filterColumns[column.id] = {
                                    value: value,
                                    type: column.dataType,
                                    // precision - we don't use it if the type is different from Decimal or Numeric
        ...
        function setFilterColumns(filter) {
            ...
            headerColumnEls
                .removeClass("slick-header-column-filtered")
                .find(".slick-filter-indicator");
               


            $.each(filterColumns, function (i, col) {
                var columnIndex = getColumnIndex(i);
                if (columnIndex != null) {
        ...
        function appendCellHtml(stringArray, row, cell, colspan, item) {
            ...
            if (item) {
                ...
                var valueFormatted = getFormatter(row, m)(row, cell, value, m, item);
                stringArray.push(valueFormatted === "" ? "" : "&#8203;" + valueFormatted + "&#8203;");
            }
